import React,{useState} from 'react'
import {Text,StyleSheet, View,TextInput,TouchableOpacity,Button} from 'react-native'
import CheckBox from '@react-native-community/checkbox';
import WalletIcon from '../Components/WalletIcon';

function Payment() {
    const [text, onChangeText] =useState("");
    const[otp,setOtp]=useState("");
    const [isSelected, setSelection] = useState(false);
    const [isSelected1, setSelection1] = useState(false);
    const [isSelected2, setSelection2] = useState(false);
    const [isSelected3, setSelection3] = useState(false);
    const[walletVisible,setWalletVisible] = useState(0);
    
    // const[active,setActive] = useState(false);
    const payui=()=>{
        setWalletVisible(1)
    }
   
   //const btn=(text)=>{
    //const phoneno = /^\d{10}$/;
    //if(phoneno.test(text)===true){
      //setWalletVisible(2)
    //}
    
     
   //}
   const verify=()=>{
    setWalletVisible(3)
   }
   
 
    return (
      <>
        <View style={styles.container}>
        <View style={styles.checkboxContainer}>
            <View style={styles.list}>
                <CheckBox
          boxType='circle'
            value={isSelected}
            onValueChange={setSelection}
            style={styles.checkbox}
          />
          <Text style={styles.label}>Credit / Debit Cards</Text>
          </View>
          
          <View style={styles.list}>
                <CheckBox
          boxType='circle'
            value={isSelected1}
            onValueChange={setSelection1}
            style={styles.checkbox}
          />
          <Text style={styles.label}>UPI</Text>
          </View>
          <View style={styles.list}>
                <CheckBox
          boxType='circle'
            value={isSelected2}
            onValueChange={setSelection2}
            style={styles.checkbox}
          />
          <Text style={styles.label}>Net Banking</Text>
          </View>
          
          <View style={styles.list} >
                <CheckBox
               
          boxType='circle'
            value={isSelected3}
            onValueChange={setSelection3}
            style={styles.checkbox}
          />
          <Text style={styles.label}>Wallet</Text>
          </View>
          
        </View>
        {isSelected3&& <View style={styles.wallet}><View style={styles.icon}>
        <TouchableOpacity onPress={payui}>
            <WalletIcon image='https://www.thehindubusinessline.com/money-and-banking/nz7hvk/article24801784.ece/alternates/FREE_615/google-paya'/>
       </TouchableOpacity> 
       <TouchableOpacity onPress={payui}>

       <WalletIcon image='data:image/png;base64,'/>
       </TouchableOpacity>
       <TouchableOpacity onPress={payui}>
        <WalletIcon image='https://static.amazon.jobs/teams/378/thumbnails/AmazonPayLogo-543.jpg?1508227956'/>

       </TouchableOpacity>
       <TouchableOpacity onPress={payui} >

       <WalletIcon  image='https://cdn2.iconfinder.com/data/icons/social-icons-color/512/paytm-512.png'/>
       </TouchableOpacity>
        
        </View></View>}
  
      
{/* <ShimmerPlaceHolder /> */}
{walletVisible===1 &&
  <View>
<Text style={{fontSize:16,marginBottom:3}}>Log in to Use your Wallet</Text>
 <Text style={{fontSize:16}}>Mobile Number</Text>
        <TextInput
        style={styles.input}
        onChangeText={onChangeText}
        value={text}
        placeholder="Mobile Number"
        keyboardType="numeric"/>
        <Button style={styles.btn}title="Send an OTP" onPress={()=>{const phoneno = /^\d{10}$/;
    if(phoneno.test(text)===true){
      setWalletVisible(2)
    }
    else{
      alert("Enter Correct Mobile Number")
    }
    }} />
        
</View>}
{walletVisible===2 &&<View>

<TextInput
        style={styles.input}
        onChangeText={setOtp}
        value={otp}
        placeholder="OTP"
        keyboardType="numeric"/>
        <Button style={styles.btn}title="Verify"  color='#146356'onPress={verify}/>
        <View style={styles.otp_box}><Text style={{fontSize:14,fontWeight:"400"}}>Didn't Receive OTP</Text>
        <Text style={{fontSize:14,color:"#541212"}}>Resend OTP</Text></View>
</View>}
{walletVisible===3 &&<View>
<Text style={{fontWeight:"500"}}>Pay Using Wallet</Text>
<Text>Available Balance : ₹3000.00</Text>
<Text>To Pay : ₹2600.00</Text>
</View>}
{walletVisible===3 &&<View style={styles.last_box}>
  <View >
  <Text style={{fontSize:19,fontWeight:'bold',color:'black'}}>Order Summary</Text>
  <Text style={{fontSize:17}}>Order Value</Text>
  <Text style={{fontSize:17}}>Product DIscounts</Text>
  <Text style={{fontSize:17}}>Loyalty points</Text>
  <Text style={{fontSize:17}}>Shipping</Text>
  <Text style={{fontSize:17,fontWeight:'bold',color:'black'}}>Grand Total</Text>
  <Text style={{fontSize:17}}>Overall Savings</Text>
  </View>
  <View>
  <Text style={{fontSize:17}}>3 ITEMS</Text>
  <Text style={{fontSize:17,fontWeight:'bold',color:'black'}}>₹7,797</Text>
  <Text style={{fontSize:17,fontWeight:'bold',color:'black'}}>₹2,657</Text>
  <Text style={{fontSize:17,fontWeight:'bold',color:'black'}}>₹657</Text>
  <Text style={{fontSize:17,color:'red'}}>FREE</Text>
  <Text style={{fontSize:17,color:'red'}}>₹2,657</Text>
  <Text style={{fontSize:17,fontWeight:'bold',color:'black'}}>₹2,657</Text>
  </View>
  
  
       
        </View>}
      </View>
      
        </>
    );
  };
  
  
  const styles = StyleSheet.create({
    container: {
      flex: 1,
      alignItems: "center",
      justifyContent: "center",
      
    },
    checkboxContainer: {
      flexDirection: "column",
      marginBottom: 20,
    },
    checkbox: {
      alignSelf: "center",
    },
    label: {
      margin: 15,
      fontSize:18,
      fontWeight:'800'

    },
    list:{
        flexDirection:'row',
        borderBottomColor: '#ddd',
        borderBottomWidth: 2},
    wallet:{
        height:100,
        width:'100%',
       
        alignItems: "center",
        justifyContent: "center",
       
        
    },icon:{
        flexDirection:'row',
        
    },
    input: {
        height: 50,
        width:200,
        margin: 12,
        borderColor:'#ddd',
        borderWidth:2,
        padding: 10,
      },
      btn:{
          height:50,
          width:250
      },
      otp_box:{
          flexDirection:'row',
          margin:4,
          justifyContent:'space-between'
      },
      last_box:{
        margin:29,
       // alignItems:'center',
        flexDirection:'row',
        flexWrap:'wrap',
        justifyContent:'space-between',
        height:220,
        width:'90%',
        backgroundColor:'#fff',
    
        
      }
  });

export default Payment